...
- `scripts/factory-run-driver.js` — the `--workflow` driver: a Workflow script stepping the same `next`/`drive` seam, wrapping every CLI call in a sonnet exec-agent (Workflow JS can't shell out)
