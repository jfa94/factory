...
description: 'Review criteria for relational schema changes — the checklist the database-design-reviewer applies to migrations and DDL in a task diff. Iron Laws (categorical) + Decision Gates (trade-offs, a claim only when unjustified and harmful) + the anti-pattern catalogue. Loaded alongside review-protocol; findings emit as RawReview claims in the v2 result envelope.'
---
...
- **Iron Laws** — categorical. A violation corrupts data or is unambiguously wrong: a claim, severity `critical` or `important`.
- **Decision Gates** — genuine trade-offs. Deviating from the default is a finding ONLY when the deviation is both unexplained (no comment/spec justification visible) and harmful at the schema's evident scale: a claim, severity `important` — unless the deviation is an Iron-Law violation in disguise.
...
A diff whose DB files are all clean → an empty `claims` row. `*.sql` files that are pure seeds/queries with no design implications are fine — do not invent findings.

## The Iron Laws (always a claim)
...
2. **Money is never binary floating point.** `FLOAT`/`DOUBLE`/`REAL`/`MONEY` near an amount → a claim. Expect DECIMAL/NUMERIC(p,s) or integer minor units, with ISO-4217 currency alongside when more than one is possible.
3. **Instants in UTC.** An event time in naive `timestamp` (no zone) → a claim. Calendar date → `DATE`; future/civil local time → local time + IANA zone name. Sentinel dates (`9999-12-31`) → a claim (also L8).
4. **One value per cell (1NF).** Comma-separated lists, `tag1/tag2/tag3` repeating columns, an array standing in for a relationship → a claim. Expect a junction/dependent table.
5. **Stated grain and a primary key.** A table whose rows mix grains, or with no PK at all → a claim.
6. **Migrations are versioned; live breaking changes use expand–contract.** A one-step destructive ALTER (`DROP`/retype/rename of a column or table that pre-existing code evidently depends on) → a claim: rolling deploys run old + new code at once. Destroying a shape the SAME diff introduced is fine. Hand-edit-style un-versioned DDL (schema change outside the repo's migration mechanism) → a claim.
7. **No EAV or polymorphic FKs by default.** Generic `(entity, attribute, value)` rows, or a `(thing_type, thing_id)` pair pointing at several tables → a claim unless the diff/spec explicitly justifies extreme sparsity. Expect typed columns, a JSONB tail for genuinely dynamic/sparse data, exclusive arcs or a shared supertype for polymorphism.
8. **NULL means unknown, never a sentinel.** `-1`, `'N/A'`, magic dates encoding "missing" → a claim. (Note: a nullable UNIQUE column admits many NULLs — flag code that assumes it caps missing values at one.)
9. **Never plaintext credentials.** A password/secret/token column with no evident hashing (bcrypt/scrypt/Argon2) on the write path → a `critical` claim.

## Decision Gates (a claim only when unjustified AND harmful)
...
Naming (snake_case, FK named after the referenced table, no reserved words) and design-level indexing (FK columns unindexed, composite order wrong for the evident access path, missing partial-unique for soft-delete) are advisory and are NOT claims — v2 has no advisory tier.
...
- `LIKE '%term%'` as the evident search plan for a new column → advisory only; not a claim unless it is an evident correctness defect.
- Every table growing `deleted_at` by reflex (G4).
- Multi-tenant tables with no `tenant_id` and no recorded tenancy decision (G9).

## Severity mapping

- Iron Law violation → a claim; L9 (credentials) and L2 (money) are `critical`, others `important`.
- Gate deviation, unjustified AND harmful → a claim, `important` (`critical` only when it is an Iron Law in disguise — e.g. a G8 miss that leaves a corruptible invariant unenforced is L1).
- Naming/indexing/style → not a claim. Never file taste.
