  ...
  if (( pb_active_len > 0 )); then
    for f in "${files[@]}"; do
      ...
      if [[ "$f" == *.json ]]; then
        ...
        raw_json=$(extract_last_json_block < "$f")
      fi
...
_run_ship_pregate() {
  ...
  case "$mut_rc:$mut_reason" in
    0:ok)                     mut_gate="ok" ;;
    0:no-package-json|0:no-script|0:no-mutable-changes)
                              mut_gate="skipped" ;;
    0:*)                      mut_gate="skipped" ;;
    *:*)                      mut_gate="fail" ;;
  esac
...
_write_ship_checklist() {
  ...
  # ci_status: from task state. Terminal failure values (timeout|closed|conflict)
  # map to "red" so the checklist reflects a non-shippable PR; only an actual
  # green CI maps to green. Anything else (e.g. literally pending) → pending.
  ...
  case "$ci_raw" in
    ...
    red|timeout|closed|conflict) ci_status="red" ;;
    *)                           ci_status="pending" ;;
  esac
