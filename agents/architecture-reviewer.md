...
Inspect the change with `git -C <taskWorktree> diff staging`, then `Read` the imports/exports
...
1. Read `CLAUDE.md`, any architecture docs, and the declared boundary config
   ...
   enforce.
2. `git -C <taskWorktree> diff staging` for scope; `Read` the imports/exports of every changed
   file.
