...
const PATH_BLOCKLIST: readonly RegExp[] = [
    /^\.env$/,
    /^\.env\..+$/,
    /\.pem$/,
    /\.key$/,
...
export type ExecFn = (command: string, args?: readonly string[], opts?: {cwd?: string}) => Promise<ExecResult>
 ...
 */
export async function decideSecretGuard(input: HookInput | null, deps: SecretGuardDeps = {}): Promise<HookDecision> {
