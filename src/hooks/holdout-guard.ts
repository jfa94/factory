...
/** Does a canonical absolute path contain a `holdouts` directory segment at any depth? */
 ...
 */
export function decideHoldoutGuard(input: HookInput | null, deps: HoldoutGuardDeps = {}): HookDecision {
    ...
    if (tool === 'Bash') {
        ...
        for (const tokRaw of bashPathTokens(cmd)) {
            ...
            const reachesHoldout =
                ...
                (dataDir != null && dataDir.length > 0 && tok.includes('holdouts') && tok.includes(dataDir))
            if (reachesHoldout) {
