...
describe('hook-io — parse', () => {
    ...
    it('readHookInput reads an injected stream', async () => {
        // A hand-rolled AsyncIterable (not `async function*`) — every chunk is
        // genuinely resolved through a Promise, exactly like a real stdin stream,
        // with no `await`-free async-generator body to trip require-await.
        function gen(): AsyncIterable<string> {
            const chunks = ['{"tool_name":', '"Write"}']
            return {
                    ...
                    return {
                        next: () => {
                            ...
                            return Promise.resolve(
                                value !== undefined ? {value, done: false} : {value: undefined, done: true}
                            )
                        },
                    }
                },
...
describe('hook-io — decisions', () => {
    ...
    it('emitBlockDecision writes the legacy {decision:block} shape', () => {
        ...
        const parsed = JSON.parse(out) as {decision: string; reason: string; detail?: string}
