...
import {handleSubagentStop, runSubagentStop, reviewerNameOf, parseVerdict, taskIdFromHeader} from './subagent-stop.js'
...
function fakeManager(initial: RunState) {
    return {
        findActiveByOwner: (_s: string) => Promise.resolve(initial),
    }
}
...
describe('handleSubagentStop — observational (NO state write)', () => {
    ...
    it('resolves task_id from the transcript [task:] header — no write', async () => {
        ...
        const readTranscript = vi.fn(() => Promise.resolve('[task:t1] reviewer transcript\nSTATUS: DONE'))
        const result = await handleSubagentStop(
            input({
                session_id: 'test-session',
...
describe('runSubagentStop — exit codes (fully observational)', () => {
    it('malformed input → OK (must not block the subagent stop)', async () => {
        const code = await runSubagentStop([], {readRaw: () => Promise.resolve('{bad')})
    ...
    it('non-reviewer role → OK', async () => {
        ...
        const code = await runSubagentStop([], {
            manager,
            readRaw: () =>
                Promise.resolve(JSON.stringify({agent_type: 'implementer', last_assistant_message: 'STATUS: DONE'})),
        })
        expect(code).toBe(EXIT.OK)
    })
