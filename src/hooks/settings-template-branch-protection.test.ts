...
function loadBranchProtectionCommand(): string {
    ...
    const entry = template.hooks.PreToolUse.find(
        (e) => e.matcher === 'Edit|Write' && (e.hooks[0]?.command.includes('Cannot edit on main/master') ?? false)
    )
    if (!entry) {
        throw new Error('branch-protection Edit|Write hook not found in template')
    }
    return at(entry.hooks, 0).command
}

function runHook(command: string, filePath: string, cwd: string): {decision?: string; reason?: string} | null {
