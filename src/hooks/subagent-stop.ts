 ...
 * derive branch.
 */
export async function handleSubagentStop(input: HookInput | null, deps: SubagentStopDeps = {}): Promise<null> {
    if (!input) {
        return null
    }
    ...
    if (!run.tasks[taskId]) {
        log.error(`resolved task_id '${taskId}' is not in run ${run.run_id}; reviewer '${reviewer}' result skipped`)
        return null
    }
