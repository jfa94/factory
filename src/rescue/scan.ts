 ...
 * stuck in-flight task — `driveRun` THROWS "dependency cycle or deadlock" the moment
...
// The driver's readiness predicates, mirrored here so scan stays decoupled from the
// driver loop. These are DEFINITIONAL (the meaning of "ready" / "blocked") and stable;
// the source of truth is src/driver/loop.ts (depsSatisfied / hasUnsatisfiableDep).
