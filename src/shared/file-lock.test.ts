...
describe('withFileLock', () => {
    it("returns fn's value and releases (a later acquire succeeds)", async () => {
        const v = await withFileLock(opts(), () => Promise.resolve(42))
        expect(v).toBe(42)
        // Lock released → re-acquire without contention.
        expect(await withFileLock(opts(), () => Promise.resolve('again'))).toBe('again')
    })

    it("dirPolicy:'assert' throws when the parent dir is missing", async () => {
        const missing = join(dir, 'nope')
        await expect(
            withFileLock(opts({dir: missing, lockfile: join(missing, 'x.lock')}), () => Promise.resolve(1))
    ...
    it("dirPolicy:'create' mkdirs the parent dir", async () => {
        ...
        await withFileLock(opts({dir: fresh, lockfile: join(fresh, 'x.lock'), dirPolicy: 'create'}), () =>
    ...
    it('releases even when fn throws (next acquire succeeds)', async () => {
        ...
        expect(await withFileLock(opts(), () => Promise.resolve('ok'))).toBe('ok')
    })
