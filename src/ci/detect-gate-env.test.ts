...
import {detectGateEnv, mergeDetectedGateEnv, applyGateEnvDetection, type WorkflowSource} from './detect-gate-env.js'
...
function fakeSource(files: Record<string, string>): WorkflowSource {
    return {
        ...
        readWorkflow: (name) => nonNull(files[name]),
    }
}
...
describe('detectGateEnv — goodbyespy golden case', () => {
    ...
    it('attributes provenance to the build step', () => {
        const url = nonNull(r.detected.find((d) => d.key === 'NEXT_PUBLIC_SUPABASE_URL'))
...
describe('mergeDetectedGateEnv — gap-fill classification', () => {
    it('writes absent keys, skips equal keys, reports differing keys as conflicts', () => {
        ...
        expect(m.conflicts).toEqual([{key: 'KEPT', configured: 'ci', detected: 'DIFFERENT', source: 'w.yml'}])
...
describe('detectGateEnv — key policy (reserved denylist + POSIX names)', () => {
    ...
    it('KEEPS NODE_OPTIONS and GIT_* — legit build/identity vars, deliberately NOT denied', () => {
        const r = detectGateEnv(
            envWorkflow(
                ['          NODE_OPTIONS: --max-old-space-size=4096', '          GIT_AUTHOR_NAME: CI Bot'].join('\n')
            )
        )
        expect(r.gateEnv).toEqual({
    ...
    it('drops non-POSIX key names → droppedKeys reason:invalid-name', () => {
        const r = detectGateEnv(
            envWorkflow(['          foo.bar: 1', '          APP-NAME: x', '          OK_NAME: y'].join('\n'))
        )
...
describe('detectGateEnv — exotic scalars (miss, never mangle)', () => {
    ...
    it('KEEPS quoted look-alikes — they are plain strings, not exotic YAML', () => {
        const r = detectGateEnv(
            envWorkflow(
                ['          DRAFT: "[draft]"', "          BANG: '!important'", '          GLOB: "*.example.com"'].join(
                    '\n'
                )
            )
        )
    ...
    it('never harvests a full-line `# KEY: value` comment inside an env block', () => {
        const r = detectGateEnv(envWorkflow(['          REAL: yes', '          # FAKE: nope'].join('\n')))
