...
describe("parseRemoteUrl", () => {
  ...
  it("parses an HTTPS URL carrying credentials and a port", () => {
    expect(parseRemoteUrl("https://user:token@github.com:443/acme/widgets.git")).toBe(
      "acme/widgets",
    );
  });
  ...
  it("collapses a nested subgroup path to the LAST two segments (owner/name)", () => {
    ...
    expect(parseRemoteUrl("https://gitlab.com/group/subgroup/widgets.git")).toBe(
      "subgroup/widgets",
    );
...
describe("resolveRepo", () => {
  ...
  it("a remote-url probe that throws (not-a-git-repo) is treated as not-derivable", async () => {
    ...
    await expect(
      resolveRepo({ explicit: "acme/widgets", cwd: "/x", gitClient: git }),
    ).resolves.toBe("acme/widgets");
