...
function lockfiles(...present: string[]): (abs: string) => Promise<boolean> {
  return async (abs: string) => present.some((p) => abs.endsWith(p));
}
...
describe("resolveSetupCommand", () => {
  ...
  it("detects `npm ci` from package-lock.json", async () => {
    expect(await resolveSetupCommand("/wt", undefined, lockfiles("package-lock.json"))).toBe(
      "npm ci",
    );
  });

  it("detects `npm ci` from npm-shrinkwrap.json", async () => {
    expect(await resolveSetupCommand("/wt", undefined, lockfiles("npm-shrinkwrap.json"))).toBe(
      "npm ci",
    );
  });
  ...
  it("FAILS LOUD when the setup command exits non-zero", async () => {
    await expect(
      provisionWorktree({
        path: "/wt",
        setupCommand: "npm ci",
        run: async () => ({ code: 1, stderr: "ENOENT lockfile" }),
      }),
  ...
  it("surfaces the command's stderr in the failure message", async () => {
    await expect(
      provisionWorktree({
        path: "/wt",
        setupCommand: "npm ci",
        run: async () => ({ code: 1, stderr: "boom-detail" }),
      }),
