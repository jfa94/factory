 ...
 */
export function runScopedBranch(runId: string, taskId: string, prefix: string = DEFAULT_PREFIX): string {
 ...
 */
export function parseRunScopedBranch(ref: string, prefix: string = DEFAULT_PREFIX): RunScopedBranchParts | null {
    const head = `${prefix}/`
    ...
    if (!isValidIdSafe(runId) || !isValidIdSafe(taskId)) {
        return null
    }
    return {prefix, runId, taskId}
}
