...
function reading(opts: {five: number; seven: number; fiveResets?: number; sevenResets?: number}): UsageReading {
    return {
        kind: 'available',
...
describe('applyQuotaGate', () => {
    ...
    it('proceed leaves a stale paused checkpoint intact (caller owns recovery)', async () => {
        ...
        // cleared by the caller — the run-level orchestrator clears it, the gate does not).
    ...
    it('ignoreQuota=true → proceeds (null) without reading usage or touching state', async () => {
        const read = vi.fn((): Promise<UsageReading> => Promise.resolve(UNAVAILABLE))
