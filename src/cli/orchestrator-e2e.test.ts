...
async function driveToTerminal(deps: OrchestratorDeps, runId: string, answer: Answerer): Promise<RunState> {
    for (;;) {
        ...
        if (next.kind === 'pause') {
            throw new Error(`unexpected quota stop: ${next.reason}`)
        }
        if (next.kind === 'document') {
            throw new Error('unexpected docs-ready in E2E helper')
        }
        if (next.kind === 'e2e') {
            throw new Error('unexpected e2e-ready in E2E helper')
        }
        if (next.kind === 'e2e-assessment') {
            throw new Error('unexpected e2e-assessment in E2E helper')
        }
        ...
        const taskId = at(next.ready, 0) // sequential orchestrator: first ready task
...
describe('runner orchestrator seam — golden contract E2E', () => {
    ...
    it('drives a task PRD→shipped (no-merge), holdout holdout recorded, panel all-approve', async () => {
        ...
        expect(nonNull(run.tasks[TASK_ID]).status).toBe('pending')
        ...
        const task = nonNull(finalRun.tasks[TASK_ID])
    ...
    it('serial-merges the task PR in live ship mode (same chain)', async () => {
        ...
        expect(taskPrs).toHaveLength(1)
        expect(gh.created).toHaveLength(2) // task PR + finalize rollup PR
        const prNum = nonNull(finalRun.tasks[TASK_ID]).pr_number
    ...
    it('fails a task at escalation cap → capability-budget → finalizeRun produces failed (Decision 34)', async () => {
        ...
        expect(at(gh.issueComments, 0).body).toContain(TASK_ID_2)
