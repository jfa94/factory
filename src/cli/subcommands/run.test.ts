...
describe('seedTasksFromSpec', () => {
    ...
    it('is LOUD on a dangling dependency', () => {
        expect(() => seedTasksFromSpec(request([task('t1', ['ghost'])]))).toThrow(/unknown task 'ghost'/)
    })
    ...
    it('is LOUD on a dependency cycle', () => {
        expect(() => seedTasksFromSpec(request([task('t1', ['t2']), task('t2', ['t1'])]))).toThrow(/dependency cycle/)
    })

    it('is LOUD on a duplicate task id', () => {
        expect(() => seedTasksFromSpec(request([task('t1'), task('t1')]))).toThrow(/duplicate task id 't1'/)
    })
})
