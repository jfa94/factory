...
describe('closed enums reject out-of-domain values (loud, not silent)', () => {
    ...
    it('rejects retired human-gate task status', () => {
        expect(() => parseRunState(minimalRun({tasks: {t1: minimalTask({status: 'needs_human_review'})}}))).toThrow()
        expect(() => parseRunState(minimalRun({tasks: {t1: minimalTask({status: 'ci_fixing'})}}))).toThrow()
    })

    it('rejects an out-of-domain failure class', () => {
        expect(() =>
            parseRunState(minimalRun({tasks: {t1: minimalTask({status: 'failed', failure_class: 'oops'})}}))
        ).toThrow()
    })
    ...
    it('rejects an out-of-domain panel verdict', () => {
        expect(() => parseTaskState(minimalTask({reviewers: [{reviewer: 'x', verdict: 'maybe'}]}))).toThrow()
    })
})
...
describe('cross-field invariants are enforced (not just documented)', () => {
    it('rejects a failed task with NO failure_class (a fail must be classified)', () => {
        ...
        expect(() => parseRunState(minimalRun({tasks: {t1: minimalTask({status: 'failed'})}}))).toThrow(/failure_class/)
    })

    it('rejects a non-failed task carrying a failure_class (set IFF failed)', () => {
        expect(() => parseTaskState(minimalTask({status: 'done', failure_class: 'spec-defect'}))).toThrow(
            /failure_class/
        )
        // default status is "pending" — still non-failed, still rejected.
        expect(() => parseTaskState(minimalTask({failure_class: 'spec-defect'}))).toThrow(/failure_class/)
    })
    ...
    it('rejects a failed task with NO failure_reason (a fail must carry a reason)', () => {
        ...
        expect(() => parseTaskState(minimalTask({status: 'failed', failure_class: 'spec-defect'}))).toThrow(
            /failure_reason/
        )
        expect(() =>
            parseTaskState(minimalTask({status: 'failed', failure_class: 'spec-defect', failure_reason: ''}))
        ).toThrow(/failure_reason/)
    })

    it('rejects a non-failed task carrying a failure_reason (set IFF failed)', () => {
        expect(() => parseTaskState(minimalTask({status: 'done', failure_reason: 'why?'}))).toThrow(/failure_reason/)
    })
    ...
    it('rejects an incoherent reviewer verdict/blocker-count pair', () => {
        ...
        expect(() =>
            parseTaskState(minimalTask({reviewers: [{reviewer: 'x', verdict: 'approve', confirmed_blockers: 2}]}))
        ).toThrow(/confirmed blocker/)
        expect(() =>
            parseTaskState(minimalTask({reviewers: [{reviewer: 'x', verdict: 'blocked', confirmed_blockers: 0}]}))
...
describe('no stored gate-verdict field exists on the schema (Δ V)', () => {
    ...
    it('strips an injected stored gate boolean (strict-by-omission)', () => {
        const run = parseRunState(minimalRun({tasks: {t1: minimalTask({quality_gate: true, mutation_gate: true})}}))
...
describe('traceability phase marker (S9, Decision 47)', () => {
    ...
    it('round-trips a done marker with met+partial verdicts (partial passes — report-surfaced)', () => {
        ...
        expect(nonNull(run.traceability).status).toBe('done')
        expect(nonNull(run.traceability).verdicts).toHaveLength(2)
        expect(at(nonNull(run.traceability).verdicts, 1).verdict).toBe('partial')
    })
    ...
    it('verdicts default to [] (no parseable audit ever landed)', () => {
        ...
        expect(nonNull(run.traceability).verdicts).toEqual([])
    })
    ...
    it('rejects an unknown verdict value and an empty requirement/evidence', () => {
        for (const bad of [
            ...
            expect(() =>
                parseRunState(minimalRun({traceability: {status: 'done', verdicts: [bad], ended_at: NOW}}))
            ).toThrow()
        }
    })
    ...
    it('Δ T4: reason is set IFF failed', () => {
        expect(() =>
            parseRunState(minimalRun({traceability: {status: 'failed', verdicts: [], ended_at: NOW}}))
    ...
    it('Δ T4: rejects a done marker carrying any unmet verdict (unmet must record as failed)', () => {
        expect(() =>
            parseRunState(minimalRun({traceability: {status: 'done', verdicts: [MET, UNMET], ended_at: NOW}}))
        ).toThrow(/unmet/)
    })
    ...
    it('round-trips a failed-with-unmet marker (verdicts persisted for the report)', () => {
        ...
        expect(nonNull(run.traceability).verdicts).toEqual([MET, UNMET])
    })
})
...
describe('TaskState.test_revision_feedback (defective-RED-test recovery)', () => {
    it('is optional (absent by default) and round-trips a stamped value', () => {
        ...
        expect(parseTaskState(minimalTask({test_revision_feedback: fb})).test_revision_feedback).toBe(fb)
    })
...
describe('e2e phase marker + author manifest', () => {
    ...
    it('rejects an unknown e2e phase status', () => {
        expect(() => parseRunState(minimalRun({e2e_phase: {status: 'weird', ended_at: NOW}}))).toThrow()
    })
    ...
    it('reason set IFF failed (mirrors DocsPhase)', () => {
        ...
        expect(() => parseRunState(minimalRun({e2e_phase: {status: 'done', reason: 'why'}}))).toThrow() // done with a reason
        const run = parseRunState(minimalRun({e2e_phase: {status: 'failed', reason: 'reopen cap exhausted for t1'}}))
        ...
        const run = parseRunState(
            minimalRun({
                e2e_assessment: {
                    ...
                    affected_specs: [{spec_path: 'e2e/x.spec.ts', task_ids: ['t1'], expectation: 'needs-update'}],
                    attempts: 1,
                    ended_at: NOW,
                },
            })
    ...
    it('e2e_assessment reason set IFF failed (T3)', () => {
        ...
        expect(() => parseRunState(minimalRun({e2e_assessment: {status: 'done', reason: 'why'}}))).toThrow() // done with a reason
        const run = parseRunState(minimalRun({e2e_assessment: {status: 'failed', reason: 'the app cannot boot'}}))
    ...
    it('rejects a bad expectation and an empty task_ids row', () => {
        ...
        expect(() =>
            parseRunState(
                minimalRun({
                    e2e_assessment: {
                        affected_specs: [{spec_path: 'e2e/x.spec.ts', task_ids: [], expectation: 'needs-update'}],
                    },
                })
            )
        ).toThrow()
    ...
    it('advisory absent when failed (the done-side counterpart of reason)', () => {
        ...
        const run = parseRunState(minimalRun({e2e_phase: {status: 'done', advisory: 'residual throwaway red'}}))
...
describe('FixFinding file/line both-or-neither (T4 mirror)', () => {
    ...
    it('accepts a full citation and an uncited finding', () => {
        expect(() => parseTaskState(minimalTask({fix_findings: [{...base, file: 'a.ts', line: 3}]}))).not.toThrow()
    ...
    it('rejects a half-citation (file without line, line without file)', () => {
        expect(() => parseTaskState(minimalTask({fix_findings: [{...base, file: 'a.ts'}]}))).toThrow(/both or neither/)
        expect(() => parseTaskState(minimalTask({fix_findings: [{...base, line: 3}]}))).toThrow(/both or neither/)
    })
})
