...
describe('panel verdict is conjunctive/unanimous (Decision 26)', () => {
    ...
    // completeness (all PANEL_ROLES present) is enforced upstream at the record
    ...
    it('a single blocked fails the panel', () => {
        expect(
            derivePanelVerdict([approve('impl'), {reviewer: 'security', verdict: 'blocked', confirmed_blockers: 1}])
                .passed
        ).toBe(false)
    })
    ...
    it('an error reviewer is NOT silently treated as approve', () => {
        expect(
            derivePanelVerdict([approve('impl'), {reviewer: 'type', verdict: 'error', confirmed_blockers: 0}]).passed
        ).toBe(false)
    })
...
describe('mergeGateBlockReason — the single shared diagnostic for a blocked merge gate', () => {
    ...
    it('names a failing deterministic gate WITH its detail (not the generic fallback)', () => {
        const reason = mergeGateBlockReason([approve('impl')], [{gate: 'type', observed: false, detail: 'tsc exit=1'}])
