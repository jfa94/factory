...
describe('isTerminalResult', () => {
    it('classifies each kind correctly', () => {
        ...
        const continuation: PhaseResult[] = [advance('tests'), spawn(request), waitRetry('ship', 'wait', 1, 3)]
        for (const r of terminal) {
            expect(isTerminalResult(r)).toBe(true)
        }
        for (const r of continuation) {
            expect(isTerminalResult(r)).toBe(false)
        }
    })

    it('throws on an unknown kind (never silently classifies)', () => {
        expect(() => isTerminalResult({kind: 'made-up'} as unknown as PhaseResult)).toThrow(/unhandled value/)
    })
})
