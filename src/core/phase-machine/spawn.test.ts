...
describe('parseSpawnRequest', () => {
    ...
    it('accepts an optional effort and rejects an empty one (loud)', () => {
        ...
        expect(() => parseSpawnRequest({resume_phase: 'exec', agents: [{...validAgent, effort: ''}]})).toThrow()
    })

    it('SpawnRequestSchema is the same validator', () => {
        expect(SpawnRequestSchema.safeParse({resume_phase: 'tests', agents: [validAgent]}).success).toBe(true)
    })
})
