...
import {assertNever, finalizeTerminal, type FinalizeTerminalResult, type PhaseResult} from './result.js'
 ...
 */
export async function runPhase(phase: EnginePhase, ctx: PhaseContext, handlers: PhaseHandlers): Promise<PhaseResult> {
...
async function dispatch(phase: EnginePhase, ctx: PhaseContext, handlers: PhaseHandlers): Promise<PhaseResult> {
...
function checkResult(phase: EnginePhase, result: PhaseResult): PhaseResult {
    ...
    if (RunPhaseEnum.safeParse(phase).success) {
        if (result.kind !== 'finalize-terminal') {
            throw new Error(
                `runPhase: run-level phase '' returned '${result.kind}' — ` +
    ...
    switch (result.kind) {
        ...
        case 'finalize-terminal':
            throw new Error(
                `runPhase: per-task phase '' returned 'finalize-terminal' — ` +
