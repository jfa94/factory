...
import {parseFinding, parseRawReview, isCitable, MAX_FINDINGS_PER_REVIEW, type Finding} from './finding.js'
...
describe('findings cap + dropped_by_cap (D43)', () => {
    ...
    it('a review over the cap is truncated to the FIRST 10 with the overflow in dropped_by_cap + a warn', () => {
        ...
        expect(at(r.findings, 0).description).toBe('finding 0')
        expect(at(r.findings, 9).description).toBe('finding 9')
