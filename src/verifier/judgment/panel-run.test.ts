...
import {derivePanelVerdict, parseTaskState, type GateEvidence, type RiskTier} from '../../types/index.js'
...
function confirmAll(holds: boolean): (r: RawReview) => FindingVerifierRunner {
    return (review) => ({
        ...
        confirm: (): Promise<VerifierVerdict> => Promise.resolve({holds, note: 'n'}),
    })
}
...
describe('WS7 panel-run integration (D26/D27, Δ K)', () => {
    it('D26 merge gate: unanimous approve → merge gate passes and advances to ship', async () => {
        ...
        if (res.result.kind === 'advance') {
            expect(res.result.to).toBe('ship')
        }
    })
...
describe('S5/A — grep-rescue meets verify-then-fix (A2 replay-keying pin)', () => {
    it('a RESCUED finding is confirmed at the CITED line (replay-verdict key) but forwarded with the RELOCATED line', async () => {
        ...
        const res = await runPanel({
            ...
            makeRunner: (review) => ({
                identity: `verifier-for-${review.reviewer}`,
                confirm: (f): Promise<VerifierVerdict> => {
                    seenLines.push(f.line)
                    return Promise.resolve({holds: true, note: 'n'})
                },
            }),
            gateEvidence: PASSING_GATES,
            phase: 'verify',
    ...
    it('never-resurrect: a DROPPED finding stays dropped even when a verdict was recorded for it', async () => {
        ...
        const res = await runPanel({
            ...
            makeRunner: (review) => ({
                identity: `verifier-for-${review.reviewer}`,
                confirm: (): Promise<VerifierVerdict> => {
                    confirmCalls++
                    return Promise.resolve({holds: true, note: 'a recorded verdict exists but must go unused'})
                },
            }),
            gateEvidence: PASSING_GATES,
            phase: 'verify',
