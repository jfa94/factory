...
export type ProbeExec = (command: string, args: readonly string[], opts: {timeoutMs: number}) => Promise<ExecResult>
...
export function makeCodexProbe(run: ProbeExec = exec): VendorProbe {
    ...
    return {
        vendor: 'codex',
        available() {
            memo ??= run('codex', ['--version'], {timeoutMs: CODEX_PROBE_TIMEOUT_MS}).then((r) => r.code === 0)
            return memo
        },
    }
}
