...
vi.mock('../../shared/index.js', async (importOriginal) => {
    const actual = await importOriginal<typeof SharedIndex>()
    return {...actual, exec: vi.fn()}
})

import {exec, at, nonNull, type ExecResult} from '../../shared/index.js'
import {
...
describe('parseE2eReport', () => {
        ...
        const report = pwJson([
            {
                suites: [
                    {
                        specs: [{title: 'nested', file: 'e2e/nested.spec.ts', tests: [{status: 'expected'}]}],
                        suites: [
                            {
                                specs: [{title: 'deep', file: 'e2e/deep.spec.ts', tests: [{status: 'unexpected'}]}],
                            },
                        ],
                    },
                ],
    ...
    it('ok is false when the process exited nonzero even though zero specs are marked failed', () => {
        const report = pwJson([{specs: [{title: 'a', file: 'e2e/a.spec.ts', tests: [{status: 'expected'}]}]}])
    ...
    it("extracts, dedupes, and ANSI-strips a failed spec's error detail (Decision 40 D8)", () => {
        ...
        const result = parseE2eReport(report)
        const failed = nonNull(result.specs.find((s) => s.status === 'failed'))
        expect(failed.error).toBe(
            "expect(locator).toBeVisible() failed\nLocator: getByRole('button')\n---\nstep timeout"
        )
        // Non-failed specs never carry error detail.
        expect(nonNull(result.specs.find((s) => s.status === 'passed')).error).toBeUndefined()
    })

    it('error detail absent when a failed test carries no results/error payload', () => {
        const report = pwJson([
            {
                specs: [{title: 'login works', file: 'e2e/login.spec.ts', tests: [{status: 'unexpected'}]}],
            },
        ])
        expect(at(parseE2eReport(report).specs, 0).error).toBeUndefined()
    })
    ...
    it('caps error detail at E2E_ERROR_DETAIL_MAX_BYTES with a truncation marker', () => {
        ...
        const error = nonNull(at(parseE2eReport(report).specs, 0).error)
...
describe('runE2e', () => {
    const okTool = (json: string): PlaywrightTool => ({
        run: () => Promise.resolve({code: 0, stdout: json, stderr: '', truncated: false}),
    })

    it('delegates to parseE2eReport on a normal (possibly red) run', async () => {
        const report = pwJson([{specs: [{title: 'a', file: 'e2e/a.spec.ts', tests: [{status: 'unexpected'}]}]}])
    ...
    it('ok is false end-to-end when the tool exits nonzero despite a clean (0-failed) report', async () => {
        ...
        const tool: PlaywrightTool = {
            run: () => Promise.resolve({code: 1, stdout: report, stderr: 'boot failed', truncated: false}),
        }
    ...
    it('throws on a truncated payload rather than parsing a clipped JSON blob', async () => {
        const tool: PlaywrightTool = {
            run: () => Promise.resolve({code: 0, stdout: '{', stderr: '', truncated: true}),
        }
...
describe('DefaultPlaywrightTool', () => {
    ...
    const found =
        ...
        () =>
            Promise.resolve(bin)
    const missing: LocalPlaywrightResolver = () => Promise.resolve(null)
    const lastCall = (): readonly [string, readonly string[] | undefined] => {
        const call = at(execMock.mock.calls, execMock.mock.calls.length - 1)
    ...
    it('passes --config when given, BEFORE the reporter flag', async () => {
        ...
        const [, args] = lastCall()
        expect(args).toEqual(['test', '--config', '/wt/.factory-e2e-throwaway.config.cjs', '--reporter=json'])
    })
