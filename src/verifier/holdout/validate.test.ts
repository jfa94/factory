...
describe("parseHoldoutVerdicts (tolerant extraction)", () => {
  ...
  it("parses a bare JSON object", () => {
    expect(parseHoldoutVerdicts(JSON.stringify(shape))).toEqual([
      { criterion: "x", satisfied: true, evidence: "e" },
    ]);
  });
...
describe("buildHoldoutPrompt", () => {
  it("lists every withheld criterion and demands the strict shape", () => {
    ...
    for (const c of record.withheld_criteria) expect(prompt).toContain(c);
