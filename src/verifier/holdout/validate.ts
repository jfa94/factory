...
/**
 * The injectable holdout-validator boundary (mirrors the panel ReviewerRunner).
 * The real v1 impl builds {@link buildHoldoutPrompt}, spawns the agent, and parses
 * via {@link parseHoldoutVerdicts}; units inject a fake. A parse failure in the
 * real impl should resolve to `[]` (every withheld criterion then scores as a
 * FAIL — fail-closed), never throw into the loop.
 */
