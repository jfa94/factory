...
describe('GateContractSchema', () => {
    ...
    it.each(GATE_IDS)("rejects a contract missing gate '%s'", (id) => {
        const raw = validContract()
        Reflect.deleteProperty(raw.gates as Record<string, unknown>, id)
    ...
    it('rejects an unknown stack and a wrong version', () => {
        expect(GateContractSchema.safeParse({...validContract(), stack: 'gradle'}).success).toBe(false)
...
describe('loadGateContract', () => {
    ...
    it('returns invalid (never absent, never throws) on broken JSON', async () => {
        ...
        if (load.state === 'invalid') {
            expect(load.error).toContain('not JSON')
        }
    })
...
describe('classifySkip', () => {
    it.each(['no-vitest-runnable-tests-in-scope', 'no-mutable-changes'])("classifies '%s' as scope", (reason) => {
