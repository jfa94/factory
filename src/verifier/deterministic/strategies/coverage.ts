...
export function resolveCoverageCommand(contract: GateContract): CoverageCommandResolution {
  ...
  if (override !== undefined) return { ok: true, cmd: { kind: "argv", argv: override } };
...
function measurementFailure(
  which: string,
  m: Exclude<CoverageMeasurement, { kind: "measured" }>,
): string {
  switch (m.kind) {
    case "command-failed": {
...
export const coverageStrategy: GateStrategy<GateTools> = {
  ...
  async run(ctx: StrategyContext<GateTools>): Promise<GateOutcome> {
    ...
    const head = await summaryFor(ctx, "head", headTree, () =>
