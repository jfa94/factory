 ...
 * Promise-returning tool signatures. Non-throwing methods drop `async` and
 * `return Promise.resolve(x)`; methods that model a failure return
 * `Promise.reject(err)` (NOT a synchronous `throw`) so `.rejects`/`.catch`
...
class FakeProcTool {
    ...
    readonly calls: {cwd: string; files?: readonly string[]}[] = []
...
export class FakeVitest extends FakeProcTool implements VitestTool {
    run(files: readonly string[], opts: ToolRunOpts): Promise<ProcResult> {
        return Promise.resolve(this.record(opts, files))
    }
}
...
export class FakeTsc extends FakeProcTool implements TscTool {
    typecheck(opts: ToolRunOpts): Promise<ProcResult> {
        return Promise.resolve(this.record(opts))
    }
}
...
export class FakeStryker implements StrykerTool {
    ...
    run(mutate: readonly string[], _opts: ToolRunOpts): Promise<StrykerResult> {
...
export class FakeCoverageTool implements CoverageTool {
    readonly measureCalls: {cwd: string; cmd: CoverageCommand}[] = []
    readonly baseCalls: {baseSha: string; cmd: CoverageCommand}[] = []
    constructor(
    ...
    measure(cmd: CoverageCommand, opts: ToolRunOpts): Promise<CoverageMeasurement> {
        ...
        return Promise.resolve(this.results.head)
    }
    measureAtBase(baseSha: string, cmd: CoverageCommand, _opts: ToolRunOpts): Promise<CoverageMeasurement> {
        this.baseCalls.push({baseSha, cmd})
        return Promise.resolve(this.results.base)
    }
}
...
export class MemoryCoverageStore implements CoverageStore {
    ...
    get(treeSha: string): Promise<CoverageSummary | null> {
        return Promise.resolve(this.entries.get(treeSha) ?? null)
    }
...
export class FakeFs implements FsProbe {
    ...
    exists(relPath: string, _opts: ToolRunOpts): Promise<boolean> {
        return Promise.resolve(this.present === null ? true : this.present.has(relPath))
    }
    existsAny(relPaths: readonly string[], _opts: ToolRunOpts): Promise<boolean> {
        const present = this.present
        if (present === null) {
            return Promise.resolve(true)
        }
        return Promise.resolve(relPaths.some((p) => present.has(p)))
    }
}
...
export class FakeGitProbe implements GitProbe {
    ...
    refExists(ref: string, _opts: ToolRunOpts): Promise<boolean> {
        return Promise.resolve(this.refs.has(ref))
    }

    revParse(ref: string, _opts: ToolRunOpts): Promise<string> {
        const sha = this.refs.get(ref)
        if (sha === undefined) {
            return Promise.reject(new Error(`fake git probe: cannot rev-parse '${ref}'`))
        }
        return Promise.resolve(sha)
    }

    treeSha(_opts: ToolRunOpts): Promise<string> {
        return Promise.resolve(this.tree)
    }

    changedFiles(_base: string, _opts: ToolRunOpts): Promise<readonly string[]> {
