...
describe('extractMutationScore', () => {
    ...
    it('returns null when the score is absent / non-numeric / non-finite', () => {
        ...
        expect(extractMutationScore({metrics: {mutationScore: Number.POSITIVE_INFINITY}})).toBeNull()
    })
...
describe('extractMutationScore — derive from schema-1.0 mutants (stock json reporter)', () => {
    ...
    it('returns null when there are zero VALID mutants (only ignored/errored)', () => {
        expect(extractMutationScore(report('CompileError', 'RuntimeError', 'Ignored', 'Pending'))).toBeNull()
    })
...
describe('Default command tools: local-bin resolution + test-gate coverage', () => {
    ...
    const found =
        ...
        () =>
            Promise.resolve(bin)
    const missing: LocalBinResolver = () => Promise.resolve(null)
...
describe('DefaultGitProbe.commits (oldest-first + merge first-parent + tag)', () => {
    ...
    function scriptGit(spec: {
        ...
        execMock.mockImplementation((_cmd: string, args: readonly string[] = []) => {
            const a = args.join(' ')
            if (a.startsWith('log --format=%H')) {
                return Promise.resolve(res(spec.shasNewestFirst.join('\n')))
            }
            if (a.startsWith('show -s --format=%P')) {
                const sha = at(args, args.length - 1)
                return Promise.resolve(res(spec.parents[sha] ?? ''))
            }
            if (a.startsWith('diff-tree')) {
                const sha = at(args, args.length - 1) // last arg is the commit in both shapes
                return Promise.resolve(res((spec.files[sha] ?? []).join('\n')))
            }
            if (a.startsWith('log -1 --format=%s%n%b')) {
                const sha = at(args, args.length - 1)
                return Promise.resolve(res(spec.message[sha] ?? ''))
            }
    ...
    it('reverses git log to OLDEST-FIRST and detects the [task-id] tag', async () => {
        ...
        expect(at(out, 0).files).toEqual(['a.test.ts'])
    })
    ...
    it('classifies a merge against its FIRST parent (parentCount > 1)', async () => {
        ...
        expect(at(out, 0).parentCount).toBe(2)
        expect(at(out, 0).files).toEqual(['merged.ts'])
        // The merge diff-tree was invoked with the FIRST parent (f1), not f2.
        const diffTreeCall = execMock.mock.calls.find((c) => c[1]?.join(' ').startsWith('diff-tree') ?? false)
    ...
    it('throws LOUD when git log output is truncated', async () => {
        execMock.mockImplementation((_cmd, args: readonly string[] = []) => {
            if (args.join(' ').startsWith('log --format=%H')) {
                return Promise.resolve(res('c1', 0, true))
            }
