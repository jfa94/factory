...
import {diffScopedTestFiles, escapeStrykerGlob, isDocsPath, isMutableSrc, isTestPath, mutationScope} from './scope.js'
...
describe('escapeStrykerGlob (glob-literal escaping for --mutate CSV)', () => {
    it('escapes [ and ] (Next.js dynamic-route segments)', () => {
        ...
        expect(escapeStrykerGlob('src/app/feedback/[token]/actions.ts')).toBe('src/app/feedback/[[]token[]]/actions.ts')
    })
    ...
    it('escapes route groups ((...))', () => {
        expect(escapeStrykerGlob('src/app/(marketing)/page.ts')).toBe('src/app/[(]marketing[)]/page.ts')
    })
