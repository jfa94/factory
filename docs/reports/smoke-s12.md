...
>   `failed`, then run `factory rescue auto` ONCE and confirm it resets the
...
| 4   | **Self-heal** — one forced task failure → failed finalize → `factory rescue auto` runs ONCE → recovered or paged correctly; `self_heal.attempts === 1`                                                                                                                                                                                                                | ⬜         |                                                                   |
