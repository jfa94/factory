...
```json
{
  "tasks": {
    "task_01": {
      ...
      "pr_status": "merged",    // reserved — never written; always "unknown" in practice
      "review_rounds": [...],
      "quality_gates": {...},
...
| `pr_status`                                 | string         | open/merged/closed — **reserved; not currently emitted by the pipeline.** `pipeline-summary` reports `unknown` until a writer exists (planned: wire from `pipeline-wait-pr` merge status). |
