...
| `developBaselineStatusChecks` | string[] | derived: `developRequiredStatusChecks` minus `"Mutation Testing"` | The at-rest baseline profile (run-scoped mode): required checks for **non-admin** PRs into develop, `strict: false`, `enforce_admins: false` — admins can push develop directly between runs. Derived default drops exactly the mutation gate, so a custom run-profile context (e.g. a repo's pgTAP check) stays required at rest; an explicitly set value (including `[]`) overrides.                                                                                                         |
