...
| `ignore_quota`              | boolean                    | When `true`, the quota gate is skipped unconditionally for this run — both coroutines + drivers read it from state (no per-call threading), mirroring the `mode==="workflow"` skip. Set at create from `--ignore-quota`, or toggled true by `factory resume --ignore-quota`. Default `false`; a legacy run without the field reads as `false`.                                    |
