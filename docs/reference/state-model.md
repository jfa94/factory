...
| `paused_minutes`            | number (default 0)           | Cumulative idle minutes deducted from wall-time by the runtime circuit-breaker so a long-paused run does not falsely trip. **Sole writer is `StateManager.update()`** (Decision 41): every write banks `max(0, gap_since_last_write − 60min)` (the `ACTIVE_GAP_CAP_MINUTES` cap), making counted runtime `Σ min(gap, 60)` — activity, not wall-clock. Absent on legacy runs → `0`.                                                                                                                                                                                                                                                                                                                           |
