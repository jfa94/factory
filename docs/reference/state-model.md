...
└── runs/
    ├── current                        # global repo-less pointer (no-cwd consumers)
    └── <run-id>/
...
- **Per-repo current pointer** — `current/<repo-key>` → `../runs/<run-id>`, in a
  ...
  this and the global repo-less `runs/current` (which serves the no-cwd consumers:
...
defaults to `[]`. Neither ever holds a gate pass/fail boolean.
