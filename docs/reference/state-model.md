...
| `owner_session`             | string?                      | Owning Claude Code session id. Scopes the Stop / SubagentStop gates (resolved via `findActiveByOwner`). **Always required at create (Decision 42)** — an ownerless `run create` is rejected as a usage error. Optional in the schema only for legacy state; an absent owner means no run is attributed to a stopping session, so the gate passes through (allow).                                                                                                                                                                                                                                                                                                                                            |
