...
quality verdict — each deterministic gate, the holdout, the floor — is recomputed
...
graph LR
  ...
  D --> V[Floor verdict<br/>computed on demand]
...
| The conjunctive **floor** verdict (gate ∧ holdout ∧ panel)   | The producer dial: `risk_tier`, `escalation_rung`    |
...
it from — so it is stored. The _floor_ (unanimity over those stored opinions) is
then derived from them (`derive.ts`).

## Where the derivation happens

`deriveFloorVerdict` is the single function that computes the floor from evidence,
...
be opened on a trusted state flag; it is opened only when the floor _re-derives_
...
- **Holdout verdicts** — stored in a verdict store because they come from an
  ...
  is the ground truth. They fold into the same `deriveFloorVerdict`.
...
- **Forgery resistance** — the central property. No agent can fake a passing gate
  by writing state; the only way to make the floor clear is to make the code
  actually clear.
...
- [Model A](./model-a.md) — derive-don't-store is what makes the brain/hands seam
  forgery-resistant.
- [The verifier and the risk-invariant floor](./verifier.md) — how the floor is
