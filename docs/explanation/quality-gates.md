...
5. Unless `quality.securityRedactFindings` is `false`, **both** gate artifacts are passed through a secret-redaction pass (the same pattern set as `hooks/secret-commit-guard.sh`) so secret-bearing source snippets lifted by the scanner are replaced with `[REDACTED]` before anything reads them: the findings artifact (scanner stdout) and the companion `<task-id>.security-gate.log` (scanner stderr — verbose/error-mode scanners can echo offending lines there). Each fails closed independently
...
| `quality.securityRedactFindings` | true    | Redact secrets from both gate artifacts at rest — the findings artifact (stdout) and the `.security-gate.log` (stderr). Set `false` to keep full fidelity for local debugging. |
