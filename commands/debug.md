...
**Autonomous mode required.** Like `/factory:run`, `/factory:debug` runs `factory autonomy status` at the top of Setup. If the session was not launched with `claude --settings <merged-settings.json>` (or `FACTORY_AUTONOMOUS_MODE=1` for CI), the check exits non-zero and the skill halts with the relaunch command (`factory autonomy ensure` prints it). The pre-launch quota gate runs only after this check clears — `usage-cache.json` is only kept fresh inside an autonomous session.
